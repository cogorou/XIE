...
namespace XIEstudio
{
	...
	public partial class CxAuxInfoForm : Form
	{
		...
		private void treeAux_Setup()
		{
			...
			#region ツリービューの初期化: (Device)
			{
				...
				KeyValuePair<string, ContextMenuStrip>[] items = 
				{
					...
					new KeyValuePair<string, ContextMenuStrip>("Grabber", GrabberMenu),
					new KeyValuePair<string, ContextMenuStrip>("DataPort", DataPortMenu),
		...
		private void treeAux_RemoveNode(TreeNode target_node)
		{
			...
			// -----
			if (ReferenceEquals(target_node, FolderNodes["Grabber"]))
			{
				#region Grabber 配下の全ての子ノード.
				{
					var nodes = new List<CxGrabberNode>();
					foreach (var node in target_node.Nodes)
					{
						if (node is CxGrabberNode)
							nodes.Add((CxGrabberNode)node);
					}
					foreach (CxGrabberNode node in nodes)
					{
						node.Reset(this.ImageView);
						node.Disconnect(CxAuxInfoForm.AuxInfo);
					}
			...
			if (target_node is CxGrabberNode)
			{
				#region Grabber 配下の選択されたノードのみ.
				{
					var node = target_node as CxGrabberNode;
					if (node != null)
					{
						target_node = null;
						node.Reset(this.ImageView);
			...
			// -----
			if (ReferenceEquals(target_node, FolderNodes["DataPort"]))
			{
				#region DataPort 配下の全ての子ノード.
				{
					var nodes = new List<CxDataPortNode>();
					foreach (var node in FolderNodes["DataPort"].Nodes)
					{
						if (node is CxDataPortNode)
							nodes.Add((CxDataPortNode)node);
					}
					foreach (CxDataPortNode node in nodes)
					{
						node.Disconnect(CxAuxInfoForm.AuxInfo);
		...
		/// イメージグラバー: ツリーノード初期化.
		/// </summary>
		private void GrabberNode_Setup()
		{
			//var nodes = XIE.Tasks.Forms.CxGrabberNode.Connect(CxAuxInfoForm.AuxInfo);
			//var folder = FolderNodes["Grabber"];
			//foreach (var node in nodes)
			//{
			//	node.ContextMenuStrip = GrabberMenu;
			//	folder.Nodes.Add(node);
			//}
			//folder.Expand();
		}
		...
		/// イメージグラバー: ツリーノード解放.
		/// </summary>
		private void GrabberNode_Dispose()
		{
			var folder = FolderNodes["Grabber"];
			foreach (var item in folder.Nodes)
			{
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void GrabberMenu_Opened(object sender, EventArgs e)
		{
			if (ReferenceEquals(treeAux.SelectedNode, FolderNodes["Grabber"]))
			{
				menuGrabberOpen.Enabled = true;
				menuGrabberClose.Enabled = true;
				menuGrabberProperty.Enabled = false;
			}
			else
			{
				var node = treeAux.SelectedNode as CxGrabberNode;
				bool enable = (node != null && node.Controller != null);
				bool is_running = (enable && node.IsRunning);

				menuGrabberOpen.Enabled = false;
				menuGrabberClose.Enabled = enable;
				menuGrabberProperty.Enabled = enable;
			}
		}

		/// <summary>
		/// イメージグラバー: オープンがクリックされたとき
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuGrabberOpen_Click(object sender, EventArgs e)
		{
			#region デバイスのオープン.
			try
			{
				var node = new CxGrabberNode();
				node.AuxInfo = CxAuxInfoForm.AuxInfo;
				node.ContextMenuStrip = GrabberMenu;
				if (node.OpenSelectDialog(this, CxAuxInfoForm.AuxInfo) == DialogResult.OK)
				{
					FolderNodes["Grabber"].Nodes.Add(node);
					FolderNodes["Grabber"].Expand();
				}
			}
			catch (System.Exception ex)
			{
		...
		/// <summary>
		/// イメージグラバー: クローズがクリックされたとき
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuGrabberClose_Click(object sender, EventArgs e)
		{
			treeAux_RemoveNode();
		}

		/// <summary>
		/// イメージグラバー: プロパティダイアログのオープン
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuGrabberProperty_Click(object sender, EventArgs e)
		{
			try
			{
				var node = treeAux.SelectedNode as CxGrabberNode;
				if (node != null)
					node.OpenPropertyDialog(this, CxAuxInfoForm.AuxInfo);
			}
			catch (System.Exception ex)
			{
				MessageBox.Show(this, ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
		...
		/// データ入出力ポート: ツリーノード初期化.
		/// </summary>
		private void DataPortNode_Setup()
		{
			//var nodes = XIE.Tasks.Forms.CxDataPortNode.Connect(CxAuxInfoForm.AuxInfo);
			//var folder = FolderNodes["DataPort"];
			//foreach (var node in nodes)
			//{
			//	node.ContextMenuStrip = DataPortMenu;
			//	folder.Nodes.Add(node);
			//}
			//folder.Expand();
		}
		...
		/// データ入出力ポート: ツリーノード解放.
		/// </summary>
		private void DataPortNode_Dispose()
		{
			var folder = FolderNodes["DataPort"];
			foreach (var item in folder.Nodes)
			{
		...
		#region データ入出力ポート: (DataPortMenu)

		/// <summary>
		/// データ入出力ポート: メニューが開いたとき
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void DataPortMenu_Opened(object sender, EventArgs e)
		{
			if (ReferenceEquals(treeAux.SelectedNode, FolderNodes["DataPort"]))
			{
				menuDataPortOpen.Enabled = true;
				menuDataPortClose.Enabled = true;
				menuDataPortProperty.Enabled = false;
			}
			else
			{
				var node = treeAux.SelectedNode as CxDataPortNode;
				bool enable = (node != null && node.Controller != null);
				bool is_running = (enable && node.IsRunning);

				menuDataPortOpen.Enabled = false;
				menuDataPortClose.Enabled = enable;
				menuDataPortProperty.Enabled = enable;
			}
		}

		/// <summary>
		/// データ入出力ポート: オープンがクリックされたとき
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuDataPortOpen_Click(object sender, EventArgs e)
		{
			#region デバイスのオープン.
			try
			{
				var node = new CxDataPortNode();
				node.AuxInfo = CxAuxInfoForm.AuxInfo;
				node.ContextMenuStrip = DataPortMenu;
				if (node.OpenSelectDialog(this, CxAuxInfoForm.AuxInfo) == DialogResult.OK)
				{
					FolderNodes["DataPort"].Nodes.Add(node);
					FolderNodes["DataPort"].Expand();
				}
			}
			catch (System.Exception ex)
			{
		...
		/// <summary>
		/// データ入出力ポート: クローズがクリックされたとき
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuDataPortClose_Click(object sender, EventArgs e)
		{
			treeAux_RemoveNode();
		}

		/// <summary>
		/// データ入出力ポート: プロパティダイアログのオープン
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void menuDataPortProperty_Click(object sender, EventArgs e)
		{
			try
			{
				var node = treeAux.SelectedNode as CxDataPortNode;
