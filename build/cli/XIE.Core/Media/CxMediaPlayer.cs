...
namespace XIE.Media
{
	...
	public unsafe class CxMediaPlayer : System.Object
		, IxModule
		, IDisposable
		, IxRunnable
		, IxParam
	{
		...
		{
			HxModule handle = xie_high.fnXIE_Media_Module_Create("CxMediaPlayer");
			...
			status = xie_high.fnXIE_Media_MediaPlayer_CreateGrabber(handle, ExMediaType.Audio, ((IxModule)AudioGrabber).GetHandle());
			if (status != ExStatus.Success)
				throw new CxException(status);

			status = xie_high.fnXIE_Media_MediaPlayer_CreateGrabber(handle, ExMediaType.Video, ((IxModule)VideoGrabber).GetHandle());
			if (status != ExStatus.Success)
				throw new CxException(status);
		...
		public void SetParam(string name, object value)
		{
			switch (name)
			{
				...
				case "StopPosition":
					{
						if (!(value is long))
							throw new System.ArgumentException("value type must be long.");
		...
		public virtual void Setup(IxModule video, IxModule audio = null, IxModule output = null)
		{
			...
			ExStatus status = xie_high.fnXIE_Media_MediaPlayer_Setup(hThis, hVideo, hAudio, hOutput);
			if (status != ExStatus.Success)
				throw new CxException(status);
		}
		...
		[CxDescription("P:XIE.Media.CxMediaPlayer.Timeout")]
		...
		#region メソッド: (Media Seeking)

		/// <summary>
		/// ストリームの時間幅を取得します。
		/// </summary>
		/// <returns>
		///		時間幅を返します。単位は 100ns です。 
		/// </returns>
		public virtual long GetDuration()
		{
			return (long)GetParam("Duration");
		}
		...
		/// </summary>
		/// <returns>
		///		現在位置を返します。単位は 100ns です。
		/// </returns>
		public virtual long GetCurrentPosition()
		{
			return (long)GetParam("CurrentPosition");
		}
		...
		/// </summary>
		/// <returns>
		///		開始位置を返します。単位は 100ns です。
		/// </returns>
		public virtual long GetStartPosition()
		{
			return (long)GetParam("StartPosition");
		}
		...
		/// </summary>
		/// <returns>
		///		停止位置を返します。単位は 100ns です。
		/// </returns>
		public virtual long GetStopPosition()
		{
			return (long)GetParam("StopPosition");
		}

		/// <summary>
		/// ストリームの開始位置を設定します。
		/// </summary>
		/// <param name="value">設定値 (100ns) [0~GetDuration()]</param>
		public virtual void SetStartPosition(long value)
		{
			SetParam("StartPosition", value);
		}
		...
		/// </summary>
		/// <param name="value">設定値 (100ns) [0~GetDuration()]</param>
		public virtual void SetStopPosition(long value)
		{
			SetParam("StopPosition", value);
		}

		#endregion

		#region メソッド: (Media Event)
		...
		///		タイムアウトすると false を返します。
		/// </returns>
		public virtual bool WaitForCompletion(int timeout)
		{
			ExBoolean result = ExBoolean.False;
			ExStatus status = xie_high.fnXIE_Media_MediaPlayer_WaitForCompletion(((IxModule)this).GetHandle(), timeout, out result);
			if (status != ExStatus.Success)
				throw new CxException(status);
			return (result == ExBoolean.True);
		}
