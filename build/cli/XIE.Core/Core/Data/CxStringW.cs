...
{
	/// <summary>
	/// 文字列オブジェクトクラス (UNICODE)
	/// </summary>
	public unsafe class CxStringW : System.Object
		, IxModule
		, IDisposable
		, ICloneable
		, IxEquatable
	{
		...
		void IxModule.SetHandle(HxModule handle, bool disposable)
		{
			...
			if (m_Handle == IntPtr.Zero)
				...
				m_Tag = (TxStringW*)m_Handle.TagPtr().ToPointer();
		}
		...
		/// </summary>
		[NonSerialized]
		private unsafe TxStringW* m_Tag = null;
		...
		///		自身のデータ構造の複製を返します。
		/// </returns>
		public unsafe virtual TxStringW Tag()
		{
			if (this.m_Tag == null)
				return new TxStringW();
			return *this.m_Tag;
		}
		...
		///		新しく生成したオブジェクトを返します。
		/// </returns>
		public unsafe static CxStringW FromHandle(HxModule handle, bool disposable)
		{
			return new CxStringW(handle, disposable);
		}
		...
		{
			HxModule handle = xie_core.fnXIE_Core_Module_Create("CxStringW");
		...
		private CxStringW(HxModule handle, bool disposable)
		{
		...
		public virtual void CopyFrom(object src)
		{
			...
			{
				var _src = (string)src;
				var _src_array = _src.ToCharArray();
				this.CopyFrom(_src_array);
				return;
			}
			if (src is IEnumerable<char>)
			{
				var _src = (IEnumerable<char>)src;
				this.Resize(_src.Count());
				this.Reset();
				var _dst = (char*)this.Address();
				int index = 0;
		...
		[CxDescription("P:XIE.CxStringW.Length")]
		...
		{
			return new string((char*)this.Address());
		}
