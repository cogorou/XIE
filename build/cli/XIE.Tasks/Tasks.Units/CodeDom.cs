	...
	class CodeLiteral
	{
		...
		public static CodeExpression From(object value)
		{
			...
			else
			{
				...
				else if (type == typeof(XIE.TxCircleArcI))
				{
					var src = (XIE.TxCircleArcI)value;
					return new CodeObjectCreateExpression(type, From(src.X), From(src.Y), From(src.Radius), From(src.StartAngle), From(src.SweepAngle));
				}
				else if (type == typeof(XIE.TxCircleArcD))
				{
					var src = (XIE.TxCircleArcD)value;
					return new CodeObjectCreateExpression(type, From(src.X), From(src.Y), From(src.Radius), From(src.StartAngle), From(src.SweepAngle));
				}
