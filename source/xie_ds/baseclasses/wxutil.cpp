...
bool TimeKillSynchronousFlagAvailable( void )
{
	...
	// The following instruction is written in the sysinfoap.h .
	// Deprecated. Use VerifyVersionInfo* or IsWindows* macros from VersionHelpers.
	//
    OSVERSIONINFO osverinfo;
    osverinfo.dwOSVersionInfoSize = sizeof(osverinfo);
	BOOL status = GetVersionEx( &osverinfo );
    if (status)
	{
	...
	BOOL status = ::VerifyVersionInfo(&osvi, dwTypeMask, dwlConditionMask);
	return (status == TRUE);
#endif
}
